...
echo " - Collection: Immediate, then every 60s"
...
while true; do
    ...
    timeout -t 60 sh -c 'for slab in $(vsish -e ls /system/fastslab/fastslabs 2>/dev/null | grep EnsSlab); do echo "Slab: $slab"; vsish -e get /system/fastslab/fastslabs/${slab}deallocTimeHisto; done' > "$TMP_SLABS" 2>&1 &
    PID3=$!
    ...
    # Skip this iteration if any of the three commands fail
    ...
    # 4. Clearance Logic (Every 3rd run)
